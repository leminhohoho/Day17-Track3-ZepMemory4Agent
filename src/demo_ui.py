...
The page shell, budget metrics and Gemini chat ship with the starter kit;
`retrieve_for_case(...)` is the student-implemented part.
...
def retrieve_for_case(
    ...
    Short-term is built from case["fixture_messages"] when present, else from
    the case's own thread in data/sessions.json (E01 has no fixture and uses
    thread minh-s1), plus any live chat turns in extra_messages. Durable layers
    are chosen from expected_layer / retrieve_layers, then everything is merged
    through the 10/4/3/3 budget.
    """
